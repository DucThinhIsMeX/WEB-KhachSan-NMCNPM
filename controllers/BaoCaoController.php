...
// Tạo báo cáo
$stmt = $this->db->prepare("INSERT OR IGNORE INTO BAOCAO_DOANHTHU (Thang, Nam, NgayLap) VALUES (?, ?, DATE('now'))");
$stmt->execute([$thang, $nam]);

$stmt = $this->db->prepare("SELECT MaBaoCao FROM BAOCAO_DOANHTHU WHERE Thang = ? AND Nam = ?");
$stmt->execute([$thang, $nam]);
$maBaoCao = $stmt->fetchColumn();
$stmt = $this->db->prepare("DELETE FROM CHITIET_BAOCAO WHERE MaBaoCao = ?");
